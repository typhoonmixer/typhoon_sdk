...
export const bitLength = (x: bigint): number => {
    ...
    let bits = 0;
    let n = x < BigInt(0) ? -x : x; // Handle negative numbers
    while (n > BigInt(0)) {
      n >>= BigInt(1);
      bits++;
    }
...
export const split128 = (a: bigint): [bigint, bigint] => {
  try{
    ...
    const MAX_UINT256 = BigInt("115792089237316195423570985008687907853269984665640564039457584007913129639936");

    const MASK_128 = BigInt((BigInt(1) << BigInt(128)) - BigInt(1));
